          ...
          <div class="mt-5 grid gap-5 lg:grid-cols-2">
            @foreach ($collaborators as $collaborator)
              ...
              <div class="card flex gap-5">
                <div class="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-ink-50">
                  @if (has_post_thumbnail($collaborator))
                    {!! get_the_post_thumbnail($collaborator, 'medium', ['class' => 'h-full w-full object-contain p-2']) !!}
                  @else
                    <span class="text-2xl font-semibold text-ink-300">{{ mb_substr(get_the_title($collaborator), 0, 1) }}</span>
                  @endif
                </div>
                <div class="min-w-0">
                  <h3 class="text-lg">
